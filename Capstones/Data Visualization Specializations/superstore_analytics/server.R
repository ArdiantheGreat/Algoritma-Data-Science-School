function(input, output, session) {
  ...
  output$product_top10_sales <- renderPlotly({
    ...
    if (selected_segment != "All"){
      filtered_plot_data <- filter(filtered_plot_data, Segment == selected_segment)
    }
    
    top10 <- head(filtered_plot_data, 10)
    plot_top10_product <- ggplot(top10, aes(y = reorder(short_name, total_sales), x = total_sales, text = tooltip)) +
      geom_bar(stat = "identity", size = 2, aes(fill = total_sales), show.legend = F) +
  ...
  output$product_top10_profit <- renderPlotly({
    ...
    top10 <- head(filtered_plot_data, 10)
    plot_top10_product <- ggplot(top10, aes(y = reorder(short_name, total_profit), x = total_profit, text = tooltip)) +
  ...
  #----data----
  output$data_table_superstore <- renderDataTable({
    datatable(ecom)
  })
}
